    ...
    public bool FindPath(int i, int j)
        {
            ...
            {
                if (Chess.Map[i, j] / 10 != CurrentPlayer && Chess.Map[i, j] % 10 != 1) 
                {
                    Blocks[i, j].Available();
                    Blocks[i, j].Enabled = true;
                }
...
<p>За да може да дефинираме кои полиња ќе бидат слободни се воведува функција FindPath() која што се повикува од сите фигури при клик на соодветната фигура, целта на оваа фунцкија е доколку во мапата од шах табелата имаме слободна позиција, односно 0, тоа значи дека тој соодветен блок треба да се означи како слободен и да се обои со портокалова боја, затоа се повикува фунцкија Available() која што го обојува тој блок.Доколку имаме фигура од спортивен играч и таа е на позиција која може да се собори (земе) од играчот и таа се означува како слободна.</p>
<p>Дел од имплементацијата на движењето на топот</p>
<pre>
...
<p>Движењата на фигурите се опишани во Moves методи, сите овие методи можат да бидат повикани и со invisible = true доколку сакаме да ги пресметаме идните позиции каде што таа фигура може да се најде, соодветно се повикува FindInvisiblePath() чија функција ја опишавме погоре.</p>
